...
<script setup>
    ...
    onMounted(async () => {
        ...
        const getCategoryId = fetchedCategories.value.find(item => item.group.replaceAll(' ', '-').toLowerCase() === groupName).id;

        try {
            const fetchCategoryById = await $fetch(`/api/category?categoryId=${getCategoryId}`);
